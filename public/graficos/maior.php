...
while($row = pg_fetch_assoc($chuvas_exec_ud_ga)){	
	...
	$chuvas_sql = "select cod_estacao, sum(valor) as qtd from public.alerta_pcd_historico where cod_estacao = '420570403A' and sensor = 'chuva' and datahora between '$min_ga' and '$max_ga' group by cod_estacao";
	$chuvas_exec = pg_query($link, $chuvas_sql);
	while($row1 = pg_fetch_assoc($chuvas_exec)){
		$qtd = $row1['qtd'];
		$cod_estacao = $row1['cod_estacao'];
		$chuvas_sql1 = "INSERT INTO public.maior (datahora_ini, datahora_fim, valor, cod_estacao) VALUES ( '$min_ga', '$max_ga', $qtd, '$cod_estacao')";
		//echo $chuvas_sql1;
